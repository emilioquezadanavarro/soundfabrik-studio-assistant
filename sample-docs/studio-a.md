# Knowledge Source: https://www.master-sound-berlin.example/studio-a

# Studio A - A space to create

A room made for full-band tracking.

With an area of approximately 70 square meters and ceiling heights of up to 5
meters, Studio A features a live room with warm, controlled acoustics. It
also houses our Yamaha C7 grand piano, fully serviced in 2023.

One additional isolation booth, measuring 12 square meters, provides separation
for vocals or a single instrument. Our control room is centered around a Solid
State Logic Origin console with ATC and Neumann monitoring.

# Selected equipment (fictional, for demo purposes only)

## Console and DAW
- Solid State Logic Origin 32-channel analog console
- AVID Pro Tools Ultimate / HDX
- Mac Studio 2023

## Monitoring
- ATC SCM25A Pro
- Neumann KH120

## Mics
- 2x Neumann U87 AI
- 1x AKG C414
- 2x Shure SM57
- 1x Shure SM7B
- 2x Sennheiser MD421

## Preamps and outboard
- 8x Neve 1073 clones
- 2x Universal Audio 1176 compressors
- 1x Tube-Tech CL1B

## Instruments
- Yamaha C7 grand piano
- Hammond M3 organ
- Gibson ES-335
- Fender Precision Bass (upon request)
